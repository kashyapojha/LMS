...
function PreviewContent({ content }) {
  switch (content.type) {
    case 'video':
      return (
        <div className="aspect-video rounded-xl bg-slate-950 border border-brand-border dark:border-slate-800 overflow-hidden">
          {content.fileUrl ? (
            <video
              src={content.fileUrl}
              controls
            ...
            <div className="flex h-full flex-col items-center justify-center gap-2 p-8">
              ...
              <p className="text-white/90 text-xs font-semibold">HTML5 Simulated Video Player</p>
              <p className="text-white/60 text-[10px]">Duration: {content.duration || '05:00'}</p>
            </div>
          )}
        </div>
      );
